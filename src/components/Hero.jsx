...
const Hero = () => {
    ...
    const handleCategoryClick = (category) => {
        ...
        navigate(`?value=${category.toLowerCase()}`);
    }
            ...
            <div className="container mx-auto max-w-[1140px] max-xl:px-5">
                ...
                <div className="flex items-center mt-[17px] max-xl:overflow-x-scroll max-xl:pb-5">
                    <div className="flex items-center gap-[5px] me-[15px]">
                        <button className={`min-w-[43px] h-[29px] text-xs leading-6 border border-solid border-black rounded-[9px] transition-all ease-linear duration-200 font-normal text-[#14191C] ${musicCategory === "All" ? "bg-black text-white" : "bg-white"}`}  onClick={() => handleCategoryClick("All")}> All</button>
                        <button className={`min-w-[47px] h-[29px] text-xs leading-6 border border-solid border-black rounded-[9px] transition-all ease-linear duration-200 font-normal text-[#14191C] ${musicCategory === "Pop" ? "bg-black text-white" : "bg-white"}`} onClick={() => handleCategoryClick("Pop")}>Pop</button>
                        <button className={`h-[29px] min-w-[54px] text-xs leading-6 border border-solid border-black rounded-[9px] transition-all ease-linear duration-200 font-normal text-[#14191C] ${musicCategory === "Rock" ? "bg-black text-white" : "bg-white"}`} onClick={() => handleCategoryClick("Rock")}>Rock</button>
                        <button className={`flex items-center gap-[5px] justify-center h-[29px] min-w-[64px] text-xs leading-6 border border-solid border-black rounded-[9px] transition-all ease-linear duration-200  font-normal text-[#14191C] ${musicCategory === "More" ? "bg-black text-white button-svg" : "bg-white "}`} onClick={() => handleCategoryClick("More")}>More <ArrowIcon /></button>
                    </div>
